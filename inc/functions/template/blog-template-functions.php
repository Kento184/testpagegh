	...
	function kalium_blog_post_date() {
		...
		if ( $show_post_date ) :
			...
			<div class="post-meta date updated published">
				...
				<?php the_time( apply_filters( 'kalium_post_date_format', 'd F Y' ) ); ?>
			</div>
			<?php
		...
		<div class="author-info">
			...
			<div class="author-info--image">
				<a href="<?php echo esc_url( $author_url ); ?>" target="<?php echo $link_target; ?>">
					<?php echo kalium_get_image_placeholder( $author_avatar ); ?>
				</a>
			</div>
			<?php endif; ?>
...
if ( ! function_exists( 'kalium_blog_post_comment_open' ) ) {
	...
	function kalium_blog_post_comment_open( $comment, $args, $depth ) {
		...
		// Comment date
		$days_format = 'l';
		$comment_date_timespan = time() - get_comment_date( 'U' );
		
		// Different years
		if ( date( 'Y', time() - $comment_date_timespan ) < date( 'Y' ) ) {
			$days_format = 'F d, Y';
		}
		// Within a week
		else if ( $comment_date_timespan > WEEK_IN_SECONDS ) {
			$days_format = 'F d';
		}
		
		$comment_date = apply_filters( 'kalium_blog_post_comment_date', sprintf( _x( '%s at %s', 'comment submit date', 'kalium' ), get_comment_date( $days_format ), get_comment_date( 'h:m A' ) ), $comment );
...
/**
 * Excerpt length when sidebar is present
 */
if ( ! function_exists( 'kalium_blog_excerpt_length_with_sidebar' ) ) {
	
	function kalium_blog_excerpt_length_with_sidebar( $length ) {
		
		if ( kalium_blog_is_in_the_loop() && kalium_blog_get_option( 'loop/sidebar/visible' ) ) {
			return 32;
		}
