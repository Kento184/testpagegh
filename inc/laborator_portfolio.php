...
function kalium_portfolio_get_lightbox_settings_and_items( $items, $gallery_id = 'portfolio-slider' ) {
	...
	if ( get_data( 'portfolio_share_item' ) ) {
		...
		if ( ! empty( $share_networks['visible'] ) ) {
			...
			//$share_networks = array_keys( $share_networks['visible'] );
			
			// Allow sharing
			$lg_options['share'] = true;
			//$lg_options['shareNetworks'] = $share_networks;
		}
	}
...
function kalium_portfolio_columns_translate_to_number( $cols ) {
	...
	if ( is_string( $cols ) ) {
		switch ( $cols ) {
			...
			// Items per row
			case '1 Item per Row':  $cols = 1; break;
			case '2 Items per Row': $cols = 2; break;
			case '3 Items per Row': $cols = 3; break;
			case '4 Items per Row': $cols = 4; break;
			case '5 Items per Row': $cols = 5; break;
			case '6 Items per Row': $cols = 6; break;
		}
	}
...
function kalium_portfolio_generate_gallery_gap( $gap = 30, $type = '' ) {
	...
	if ( $type == 'carousel' ) {
		...
		generate_custom_style( '.single-portfolio-holder .gallery .photo, .single-portfolio-holder .portfolio-video-holder, .single-portfolio-holder .gallery .portfolio-images-slider, .single-portfolio-holder .gallery .comparison-image-slider', "margin-bottom:{$gap}px;" );
